...
/*Removes featured image from portfolio posts.
...
add_filter( 'beans_post_more_link_text_output', 'seclift_modify_read_more' );
function seclift_modify_read_more() {
 return 'Read More..';
}
/*
...
add_action( 'wp', 'seclift_posts_grid' );

function seclift_posts_grid() {
...
function beans_child_modify_post_content( $content ) {
    ...
    return sprintf(
        '<p>%s</p><p>%s</p>',
        has_excerpt() ? get_the_excerpt() : wp_trim_words( $content, 80, '...' ),
